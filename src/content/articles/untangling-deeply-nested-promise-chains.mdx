...
The following code is a so-called "basic" example of how use Service Worker to implement a _network-first with cache fallback_ strategy for offline support. This exact code (or a variation of it) can be found in numerous tutorials on the web:
...
While I suppose this code could be considered "basic" in the sense that it's only a few lines and the strategy is conceptually simple, I'd argue that the control flow makes it anything but basic. Unless you're _very_ familiar with the new [`Fetch`](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API) and [`CacheStorage`](https://developer.mozilla.org/en-US/docs/Web/API/CacheStorage) APIs, and unless you have a solid understanding of all the [nuances of promises](https://pouchdb.com/2015/05/18/we-have-a-problem-with-promises.html), it's probably going to take you a few reads to really grok how this code works.
...
> [!NOTE]
> My intent here is not to critique people who have written Service Worker tutorials. I've learned a great deal from that content, and I think it's invaluable. I also understand that when writing blog posts, concise examples often have a place.
>
...
```js /networkResponse/ /cacheResponse/
self.addEventListener('fetch', (event) => {
  event.respondWith(
    caches.open('cache:v1').then((cache) => {
...
If you have an expression that's so long/complex that you feel the need to visually separate it, why not _actually_ separate it: evaluate the expression elsewhere and assign the result to a variable.

```js /networkOrCacheResponse
self.addEventListener('fetch', (event) => {
...
As it stands in the above code example, the `networkOrCacheResponse` object can only be used inside this particular `fetch` handler. If you wanted to use the _network-first with cache fallback_ strategy elsewhere, you'd have to rewrite the logic.
...
```js /addToCache/ /getCacheResponse/
...
While at first it might seem like this abstraction will end up doing more work, it's actually an optimization over the original code&mdash;one I only discovered _after_ I started separating the concerns.
...
Async functions have been around for a while now, and you can compile them to ES5 using [Babel](http://babeljs.io/). However, in order to use that compiled code you _also_ have to include the `babel-polyfill` library (which bundles Facebook's [regenerator](https://github.com/facebook/regenerator) runtime).
