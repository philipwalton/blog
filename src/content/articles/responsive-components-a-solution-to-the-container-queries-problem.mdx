...
> [!NOTE]
> The `:matches()` selector isn't well supported in current browsers. However, you can use tools like [postcss-selector-matches](https://www.npmjs.com/package/postcss-selector-matches) to transpile `:matches()` into something the works cross-browser.
