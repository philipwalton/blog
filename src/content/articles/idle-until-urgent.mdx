...
> [!IMPORTANT]
> While browsers can run input callbacks ahead of queued tasks, they _cannot_ run input callbacks ahead of queued microtasks. And since promises and `async` functions run as microtasks, converting your sync code to promise-based code will not prevent it from blocking user input!
>
...
```js /setTimeout/ /requestIdleCallback/
const main = () => {
...
```js /this.formatter/ /this.formatter.format/
class MyComponent {
  constructor() {
