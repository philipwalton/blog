...
> [!NOTE]
> As an alternative to passing the `EXPERIMENT` variable via your JavaScript bundler (which will require you to generate separate bundles per experiment) you can set it as a global variable via a `<script>` tag in your HTML templates. This is a good idea if you're not testing any other JavaScript features and you don't need your JS files to change between experiments. See more on this below in Tips and key learnings.
...
> [!NOTE]
> This code is a slight simplification from the code I'm actually using on my blog. If you're curious to see the full logic (which includes some unrelated things), check out my [worker source on GitHub](https://github.com/philipwalton/blog/blob/c82ea9fbc1d6be3ea317a312e5bde17d69dd53d7/worker/index.js).
...
> [!NOTE]
> You might be wondering why the Render Time values and TTFB values don't add up to the FCP value in the table above. The reason is that p75 Render Time equals `p75(FCP - TTFB)`, which is not the same as `p75(FCP) - p75(TTFB)`.
