...
Taking the time to figure out *why* your hack works may seem costly now, but I promise it'll save you time in the future. Having a fuller understanding of the systems you're building within will mean less guess-and-check work going forward.
...
In this case IE6 was the catchall for IE versions, presumably to make versions of IE older than 6 act like IE6, but at soon as IE10 came out, large portions of our application completely broke.
...
A timely example of this is the recent change to the CSS Flexbox specification where flex items now have their default `min-width` and `min-height` set `auto`, which means they shouldn't shrink below their minimum content size.<sup>[[1]](#footnote-1)</sup> For almost the past 8 months, Firefox was the only browser to implement this spec change.<sup>[[2]](#footnote-2)</sup>

If you encountered the cross-browser incompatibility and noticed that your site rendered the same in Chrome, IE, Opera, and Safari, but looked different in Firefox, you may very well assume Firefox has it wrong. In fact, I've witnessed this happen a lot. Many of the issues reported on my [Flexbugs](https://github.com/philipwalton/flexbugs) project were actually due to this spec change, and the workarounds proposed, if implemented, would have failed two weeks ago when Chrome 44 came out. Instead of these workarounds following the spec, they were unknowingly penalizing good behavior.

If you take the time to understand what's supposed to happen and write your code with that in mind, your workarounds will be far more future-proof.
...
I get the impression that there are a lot more front-end developers who want to freelance (or otherwise work full-time by themselves) than there are back-end developers with the same goal. Perhaps its because front-end people tend to be self-taught and back-end people tend to come from academia.
...
If you do end up working for yourself at some point in your career, make a point of becoming (or staying) involved in open source. Actively contributing to open-source projects gives you many of the same benefits of working on a team, sometimes even more benefits.

## Reinvent the wheel

Reinventing the wheel is bad for business, but it's great for learning. You may be tempted to grab that typeahead widget or event delegation library from npm, but imagine how much more you'd learn trying to build that thing yourself.

I'm sure some people reading this are getting pretty uncomfortable right about now. Don't get me wrong. I'm not saying you should never use third-party code. Using well-tested libraries that have the advantage of years of test cases and bug reports is almost always the smart thing to do.

But in this article I'm talking about how to go from good to great. Most of people I consider "great" in this industry are the creators or maintainers of very popular libraries that I use all the time.

You could probably have a very successful career without ever building your own JavaScript library, but you'll probably also never work close enough to the metal to really get your hands dirty.
...
<aside class="Footnotes">
  ...
  <ol class="Footnotes-items">
    <li id="footnote-1">You can see examples of this bug in my <a href="https://github.com/philipwalton/flexbugs">Flexbugs</a> project under <a href="https://github.com/philipwalton/flexbugs#1-minimum-content-sizing-of-flex-items-not-honored">flexbug #1</a>.</li>
    <li id="footnote-2">Firefox implemented the spec change in <a href="https://en.wikipedia.org/wiki/Firefox_release_history">version 34</a> on December 1, 2014. Google Chrome implemented it in <a href="https://en.wikipedia.org/wiki/Google_Chrome_release_history">version 44</a> on July 21, 2015.</li>
  </ol>
</aside>
