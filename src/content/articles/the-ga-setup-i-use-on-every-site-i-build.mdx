...
> [!NOTE]
> The `nonInteraction` field is set to true to prevent this event from influencing bounce rate calculations. If you're curious as to why this is important you should read more about [non-interaction events](https://support.google.com/analytics/answer/1033068#NonInteractionEvents).
...
```js {1[36-42],2[36-42],4[5-9],5[5-9]}
...
> [!TIP]
> To get median values in Google Analytics, all you have to do is add the _Hit ID_ dimension to your report, sort the metric values in ascending order, and then read the middle value.
