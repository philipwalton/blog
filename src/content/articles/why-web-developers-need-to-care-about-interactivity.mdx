...
excerpt: 'Anyone who’s browsed the web on their phone has, at one point or another, experienced this situation: You open a web page and click on something, but nothing happens. You click on it again—still nothing.'
translations:
...
> [!NOTE]
> Some browsers are now able to render animated GIFs while the main thread is blocked; however, that has not historically been the case, and not all browsers currently support this feature.
...
> [!WARNING]
> You might suddenly find yourself on a new page once the main thread is unblocked!
